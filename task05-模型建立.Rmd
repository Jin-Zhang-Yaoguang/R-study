...
Task02共计3个知识点，预计需学习2-3小时，请安排好学习任务。
...
summary(lr_model)
```
运用plot命令对模型进行诊断，各图含义参考https://www.cnblogs.com/lafengdatascientist/p/5554167.html
...
对于逐步回归，一般有前向、后向、双向等逐步方式。本部分将基于已经实现的`lr_model`进行双向逐步回归。前向和后向回归只需要更改`step()`命令行中的`direstion`参数即可。具体内容参照https://blog.csdn.net/qq_38204302/article/details/86567356
